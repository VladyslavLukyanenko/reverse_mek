...
const vm = require('vm');
const processor = require("./processors/block.processor");
...
const script = fs.readFileSync('mek.js', 'utf-8');
...
traverse(AST, {
  ObjectProperty(path) {
    switch (path.node.value.type) {
      case "StringLiteral": {
        simpleOpWrapFunctions[path.node.key.value] = path.node.value;
        break;
...
traverse(AST, {
  MemberExpression(path) {
    if (path.node.property?.value?.length !== 5) return;
    const value = simpleOpWrapFunctions[path.node.property.value];
    switch (value?.type) {
      case "FunctionExpression": {
      ...
      case "StringLiteral": {
        if (path.parent.type === "AssignmentExpression") return;
        path.replaceWith(value)
        break;
      }
    }
  ...
  ObjectProperty(path) {
    switch (path.node.value.type) {
      ...
      case "MemberExpression": {
        if (path.node.value.property?.value?.length !== 5) break;
...
const numericLiteralsHexToDec = {
  NumericLiteral(path) {
    delete path.node.extra.raw;
  }
}

traverse(AST, numericLiteralsHexToDec);

traverse(AST, {
  IfStatement(path) {
    const curr = path.node;
...
// make cases to have block statement as consequent part
traverse(AST, {
  SwitchCase(path) {
...
const isTopLvlObjExpDecl = (path, state) => {
  ...
  if (path) {
    ...
    if (path.parentPath) {
      return isTopLvlObjExpDecl(path.parentPath, state);
    }
  }

  return state.fnsCount === 1;
}
traverse(AST, {
  BlockStatement(path) {
    for (let ix = 0; ix < path.node.body.length; ix++) {
      const c = path.node.body[ix];
      if (types.isVariableDeclaration(c)) {
        if (c.declarations.length <= 1) {
          continue;
        }

        const decls = c.declarations.slice();
        const output = decls.map(d => ({type: "VariableDeclaration", declarations: [d], kind: c.kind}));
        // processor.simplifySequenceExpr(c, output);
        path.node.body.splice(ix, 1, ...output);
        ix--;
      }
    }
  },
  ObjectProperty(path) {
    const objDecl = path.parentPath?.parentPath;
    if (!objDecl) {
      return;
    }

    const name = objDecl.node?.id?.name;
    if (!name) {
      return;
    }
    ...
    const target = objDecls[name];
    const propName = types.isIdentifier(path.node.key) ? path.node.key.name : path.node.key.value;
    const propValue = JSON.parse(JSON.stringify(path.node.value));
    /* if (propName in target) {
       console.warn(propName);
     }*/

    if (propValue.type === "FunctionExpression") {
      if (propName in fnDecls) {
        console.warn("Fn exists " + propName);
      }

      fnDecls[propName] = propValue;
    } else {
      if (propValue.type.endsWith("Literal") && isTopLvlObjExpDecl(path)) {
        if (propName in literalDecls) {
          console.warn("Literal exists " + propName);
        }
        literalDecls.set(propName, propValue);
      }
    }
...
// console.log(literalDecls);
let convertedFromComputed = false;
do {
  convertedFromComputed = false;
  traverse(AST, {
    MemberExpression(path) {
      const node = path.node;
      if (node.computed
        && types.isStringLiteral(node.property)
        && validMemberNameRegex.test(node.property.value)
        && isNaN(+node.property.value)
      ) {
        node.computed = false;
        node.property = types.identifier(node.property.value)
        convertedFromComputed = true;
      }
    }
  });
  if (convertedFromComputed) {
    traverse(AST, {
      CallExpression(path) {
        const node = path.node;
        if (node.callee.type !== "MemberExpression"
          || node.callee.property.type !== "Identifier"
          || !(node.callee.property.name in fnDecls)
        ) {
          return;
        }

        const wrapFunction = fnDecls[node.callee.property.name];
        if (wrapFunction.type !== "FunctionExpression") {
          return;
        }


        let json = JSON.stringify(wrapFunction);
        if (!json) {
          return;
        }
        ...
        let replacer;
        try {
          replacer = JSON.parse(json);
        } catch (e) {
          console.error("Invalid json " + json)
          throw e;
        }
        const retStmtArg = replacer.body.body[replacer.body.body.length - 1]?.argument;

        const was = generate(node).code;
        let replaced = false;
        for (let ix = 0; ix < node.arguments.length; ix++) {
          const arg = node.arguments[ix];
          const param = replacer.params[ix];
          if (!param) {
            continue;
          }
          ...
          const replaceVisitor = {
            noScope: true,
            Identifier(idPath) {
              if ("__processed__" in idPath.node) {
                idPath.skip();
                return;
              }
              ...
              idPath.scope = path.scope;
              arg.scope = path.scope;
              traverse(arg, {
                noScope: true,
                enter: function (e) {
                  e.node.__processed__ = true;
                }
              });
              replaced = true;
              idPath.replaceWith(arg);
            }
          };
          traverse(retStmtArg, replaceVisitor);
        }

        if (replaced) {
          const became = generate(retStmtArg).code;
          // console.log(`${was} -> ${became}`);
          path.replaceWith(retStmtArg);
        }
      }
    });
  }
} while (convertedFromComputed);
...
traverse(AST, {
  BlockStatement(path) {
    ...
    // todo: process all vardecls because they can be located ot only at index `1`
    const varDecl = block.body[1];
    const decl = varDecl?.declarations && varDecl.declarations[0];
    if (!types.isVariableDeclaration(varDecl) || !types.isObjectExpression(decl?.init)) {
      return;
    }

    const literalsToInline = new Map();
    traverse(decl.init, {
      noScope: true,
      ObjectProperty(path) {
        const objDecl = path.parentPath?.parentPath;
        ...
        const propName = types.isIdentifier(path.node.key) ? path.node.key.name : path.node.key.value;
        if (types.isLiteral(path.node.value)) {
          try {
            const propValue = {type: path.node.value.type, value: path.node.value.value};
            literalsToInline.set(propName, propValue);
          } catch (e) {
            console.error(e);
          }
        }
    ...
    const membersToInspect = block.body.slice(2);

    let noReplacements = true;
    do {
      noReplacements = true;
      for (const s of membersToInspect) {
        traverse(s, {
          noScope: true,
          MemberExpression(path) {
            const memb = path.node;
            if (
              types.isIdentifier(memb.property)
              && types.isIdentifier(memb.object)
              && memb.object.name === decl.id.name
              && literalsToInline.has(memb.property.name)
            ) {
              try {
                path.replaceWith(literalsToInline.get(memb.property.name));
              } catch (e) {
                console.error(e);
              }
...
// reduce `if` nesting if contains only single `if` in `else` stmt
traverse(AST, {
  IfStatement(path) {
    const node = path.node;
    if (types.isBlock(node.alternate) && node.alternate.body.length === 1 && types.isIfStatement(node.alternate.body[0])) {
